...
| 인증 | JWT access token, 해시로 보관하는 refresh token, 갱신 시 토큰 교체, 로그아웃 시 refresh token 폐기 |
...
검증 대상은 토큰 생성·만료·변조, refresh token 교체, 소셜 로그인 정보, 메모 소유권, 이미지 형식·크기. PostgreSQL 17 격리 환경에서도 API 19개 항목과 동시 refresh 요청 검증 완료. 로그인·회원가입 요청 제한은 격리 프록시 20개·운영 9개 항목 검증 후 적용. 실기기 카카오 로그인·책·메모 저장은 사용자 확인 완료. 실제 푸시 수신과 Apple 로그인은 추가 확인 대상. [운영 적용·검증 범위](docs/auth-and-access.md)
